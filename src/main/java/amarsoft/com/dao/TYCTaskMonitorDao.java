 ...
 */
@Component
public class TYCTaskMonitorDao {
    private final Logger logger = LoggerFactory.getLogger(TYCTaskMonitorDao.class);
    ...
    public TYCTaskMonitor selectMonitorBySerialno(String serialno) {
        String sql = "select * from tyc_task_monitor where serialno=?";
    ...
    public TYCTaskMonitor selectNewestMonitor(String tablename) {
        String sql = "select * from tyc_task_monitor where tablename=? order by inputtime desc limit 1";
    ...
    public void updateMonitorBySerialno(String serialno, Integer status, String message) {
        String sql = "update  tyc_task_monitor set status=?, message = ? where serialno =? ";
    ...
    public void updateMonitorParams(String serialno, String params, Integer status, String message) {
        String sql = "update tyc_task_monitor set inputparams = ? , status =? ,message = ? where serialno=?";
    ...
    public void insertTaskMonitor(TYCTaskMonitor t) {
        ...
        String sql = "INSERT INTO tyc_task_monitor (" + keys + ") VALUES " + "(" + values + ")";
